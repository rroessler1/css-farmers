...
def run_model(steps=100):
    ...
    for i in range(steps):
        ...
        if (i + 1) % 10 == 0:
            ...
            print(f"\nStep {i + 1}:")
            print(f"  Farmers with biogas plants: {int(last_row['Farmers with Plants'])}/{int(last_row['Total Farmers'])}")
            print(f"  Total biogas plants: {int(last_row['Total Biogas Plants'])}")
            print(f"  Total money distributed: ${last_row['Total Money Distributed']:.2f}")
    ...
    print(f"  Farmers with plants: {int(final_row['Farmers with Plants'])} ({int(final_row['Farmers with Plants'])/int(final_row['Total Farmers'])*100:.1f}%)")
    ...
    if not agent_data.empty:
        ...
        if not farmers_data.empty:
            ...
            print(f"  Average money received: ${farmers_data['Money Received'].mean():.2f}")
